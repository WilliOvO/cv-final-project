...
class FlowModule(LightningModule):
    ...
    def model_step(self, noisy_batch: Any):
        ...
        return {
            ...
            "control_field": model_output,  
            "r3_t": r3_t,
            "so3_t": so3_t,
            "loss_mask": loss_mask
        }

    def compute_adjoint_matching_loss(self, batch_losses):

        
        control_field = batch_losses["control_field"]

        r3_t = batch_losses["r3_t"]

        sigma_t = torch.sqrt(2.0*torch.clamp(r3_t, min=1e-3)) 
        

        a_t = self.compute_lean_adjoint_state(batch_losses, sigma_t)  

        # LAdj−Match = 1/2 * E[∫ (u(X,t)+σ(t)*a(t))² dt]

        u_trans = control_field["pred_trans"]
  
        diff = u_trans + sigma_t[..., None]*a_t
        adj_loss = torch.mean(diff**2)
        return adj_loss

    def compute_lean_adjoint_state(self, batch_losses, sigma_t):
        # TODO: SOC,adjoint ODE, calculate a_t
        control_field = batch_losses["control_field"]
        u_trans = control_field["pred_trans"]
        a_t = torch.zeros_like(u_trans)
        return a_t
    ...
    def training_step(self, batch: Any, stage: int):
        ...
        total_losses = {
            k: torch.mean(v) for k,v in batch_losses.items() if k not in ["control_field","r3_t","so3_t","loss_mask"]
        }

        adj_loss = self.compute_adjoint_matching_loss(batch_losses)
        final_loss = total_losses['se3_vf_loss'] + adj_loss
        self._log_scalar("train/adjoint_matching_loss", adj_loss, batch_size=num_batch)

        for k,v in total_losses.items():
            self._log_scalar(
                f"train/{k}", v, prog_bar=False, batch_size=num_batch)
        
        so3_t = torch.squeeze(batch_losses['so3_t'])
        self._log_scalar(
            "train/so3_t",
            np.mean(du.to_numpy(so3_t)),
            prog_bar=False, batch_size=num_batch)
        r3_t = torch.squeeze(batch_losses['r3_t'])
        self._log_scalar(
            "train/r3_t",
            np.mean(du.to_numpy(r3_t)),
            prog_bar=False, batch_size=num_batch)
        for loss_name, loss_dict in batch_losses.items():
            if loss_name in ['control_field','r3_t','so3_t','loss_mask']:
                continue
        ...
        self._log_scalar(
            "train/loss", final_loss, batch_size=num_batch)
        return final_loss
